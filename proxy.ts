...
export async function proxy(req: NextRequest) {
  ...
  // Already logged in — redirect auth/public pages to dashboard
  if (PUBLIC_PATHS.has(pathname)) {
    return redirectTo(req, "/dashboard");
  }

  // ops cannot access /admin section
...
export const config = {
  matcher: ["/((?!api|_next/static|_next/image|favicon\\.ico|icon\\.png|BearthProducts).*)"],
};
