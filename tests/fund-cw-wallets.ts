/**
 * One-shot script: fund CW1–CW5 with Sepolia ETH from the operations wallet.
 * Run: npx ts-node tests/fund-cw-wallets.ts
 * Requires OPS_PRIVATE_KEY env var (operations wallet with enough Sepolia ETH).
 */
...
async function main() {
  ...
  for (const cw of WALLETS) {
    ...
    console.log(`${cw.name} current balance: ${ethers.formatEther(before)} ETH`);
