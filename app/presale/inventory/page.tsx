...
export default function InventoryPage() {
  ...
  const loadOverview = useCallback(() => {
    ...
    fetch("/api/presale/inventory", { credentials: "include" })
      .then(r => r.json())
  ...
  const loadPOs = useCallback((status = posFilter) => {
    ...
    fetch(`/api/presale/inventory/purchase-orders?${params}`, { credentials: "include" })
      .then(r => r.json())
  ...
  const loadMovements = useCallback((offset = 0) => {
    ...
    fetch(`/api/presale/inventory/stock-movements?limit=50&offset=${offset}`, { credentials: "include" })
      .then(r => r.json())
  ...
  const loadReturns = useCallback((status = returnsFilter) => {
    ...
    const params = new URLSearchParams({ limit: "50", offset: "0" });
    if (status) params.set("status", status);
    fetch(`/api/presale/inventory/returns?${params}`, { credentials: "include" })
      .then(r => r.json())
  ...
  const openPODetail = async (po: PurchaseOrder) => {
    const res = await fetch(`/api/presale/inventory/purchase-orders/${po.id}`, { credentials: "include" });
  ...
  const handleCreatePO = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/presale/inventory/purchase-orders", {
  ...
  const openReceivePO = (po: PurchaseOrder) => {
    setShowReceivePO(po);
    fetch(`/api/presale/inventory/purchase-orders/${po.id}`, { credentials: "include" })
      .then(r => r.json())
      .then(d => {
  ...
  const handleReceivePO = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/presale/inventory/purchase-orders/${showReceivePO.id}/receive`, {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
  ...
  const handleCreateReturn = async () => {
    ...
    try {
      const res = await fetch("/api/presale/inventory/returns", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleProcessReturn = async () => {
    ...
    setReturnSaving(true); setReturnError(null);
    try {
      const res = await fetch(`/api/presale/inventory/returns/${processingReturn.id}/process`, {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
