...
export default function FulfillmentPage() {
  ...
  const loadFulfillments = useCallback((status = filterStatus, type = filterType, off = offset) => {
    ...
    fetch(`/api/presale/fulfillment?${params}`, { credentials: "include" })
      .then(r => r.json())
  ...
  const openDetail = async (f: Fulfillment) => {
    ...
    try {
      const res = await fetch(`/api/presale/fulfillment/${f.orderId}`, { credentials: "include" });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/presale/fulfillment/${editModal.orderId}`, {
  ...
  const handleInitialize = async () => {
    ...
    try {
      const res = await fetch("/api/presale/fulfillment/initialize", {
        method: "POST", credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({}),
      });
