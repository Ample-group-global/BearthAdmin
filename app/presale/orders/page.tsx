...
function OrdersPageInner() {
  ...
  useEffect(() => {
    fetch("/api/presale/master", { credentials: "include" }).then(r => r.json()).then(d => setMaster(d)).catch(() => {});
    fetch("/api/presale/customers?limit=500&offset=0", { credentials: "include" }).then(r => r.json()).then(d => setCustomers(d.customers ?? [])).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    fetch("/api/presale/nft?limit=500&offset=0", { credentials: "include" })
      .then(r => r.json()).then(d => setNftRecords(d.nftRecords ?? [])).catch(() => {});
    fetch("/api/presale/products?limit=500&offset=0", { credentials: "include" })
  ...
  const openCreate = async () => {
    ...
    try {
      const res = await fetch("/api/presale/orders/next-number", { credentials: "include" });
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/presale/orders/${confirmModal.orderId}`, {
