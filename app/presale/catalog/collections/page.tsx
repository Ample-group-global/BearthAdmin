...
export default function CollectionsPage() {
  ...
  const load = useCallback((bId = filterBrand, q = search) => {
    ...
    fetch(`/api/presale/catalog/collections?${params}`, { credentials: "include" })
      .then(r => r.json())
  ...
  useEffect(() => {
    load();
    fetch("/api/presale/catalog/brands?limit=100", { credentials: "include" })
  ...
  const handleSave = async () => {
    ...
    try {
      const url = modal === "edit" ? `/api/presale/catalog/collections/${editing!.id}` : "/api/presale/catalog/collections";
