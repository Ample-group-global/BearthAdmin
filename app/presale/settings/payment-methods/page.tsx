...
export default function PaymentMethodsPage() {
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch("/api/presale/payment-methods", { credentials: "include" })
      .then(r => r.json())
  ...
  const handleEdit = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/presale/payment-methods/${editMethod.id}`, {
  ...
  const handleToggle = async (m: PaymentMethod) => {
    await fetch(`/api/presale/payment-methods/${m.id}`, {
      method: "PUT", credentials: "include",
      headers: { "Content-Type": "application/json" },
