...
// LEGACY FALLBACK — kept for reference during migration; not used when API is available.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const _STRATEGIES_LEGACY: Partial<Strategy>[] = [
  {
    priority: 1,
  ...
  },
  {
    ...
    description:
      "Price starts at a high point and automatically drops at set intervals until NFTs are sold or the floor price is reached. Buyers decide when the price is right for them. Eliminates the guesswork of fixed pricing and prevents under-pricing or over-pricing a new collection. The Bearth smart contract has full on-chain Dutch Auction support — any wave can run a Dutch Auction.",
    process: [
      { step: 1, title: "Set Wave Sale Method to Dutch Auction", detail: "In NFT Waves page, edit the wave and set Sale Method to 'Dutch Auction'. This marks the wave's strategy in the DB." },
      { step: 2, title: "Configure Dutch Auction Parameters On-Chain", detail: "In the wave's On-Chain Actions modal, go to Section 5 (Configure Dutch Auction). Enter: Start Price (ETH), Floor Price (ETH), Decrement per interval (ETH), Interval (seconds). Submit — calls setDutchAuction() on the contract." },
      { step: 3, title: "Set Wave Schedule", detail: "Set the wave's start and end times in the On-Chain Actions modal. The contract enforces minting only within this window." },
      { step: 4, title: "Open the Auction", detail: "Launch at the announced time. Price begins dropping automatically on-chain. Buyers call dutchMint() — the price they pay is the current on-chain price at their transaction moment." },
      { step: 5, title: "Monitor Current Price", detail: "In the wave's On-Chain Actions modal, the current Dutch Auction price is displayed live. Share price updates on social to drive buying urgency as the price drops." },
      { step: 6, title: "Close and Record", detail: "When sold out or window closes, close the wave via treasury (mint remaining to treasury) or burn. Create BearthAdmin orders for all Dutch Auction purchases to track fulfillment." },
    ],
    bestFor: ["Rare / Legendary tier NFTs", "New waves", "Price discovery", "Transparent pricing"],
    tips: [
  ...
  },
  {
    ...
    description:
      "NFT holders stake (lock) their Bearth NFT in a staking contract to earn rewards over time. Staked NFTs cannot be sold until unstaked. This reduces circulating supply on OpenSea, supports floor price, and keeps the community actively engaged with the project. The Bearth BearthStaking contract is deployed with rarity multipliers, genesis bonuses, and on-chain BRT token redemption.",
    process: [
      { step: 1, title: "Configure Staking in Admin", detail: "Go to NFT Staking page. Enter the deployed BearthStaking contract address and RewardToken (BRT) address in the config section. Set the points-per-token rate and enable staking." },
      { step: 2, title: "Set Rarity Multipliers", detail: "In the staking config, set BPS multipliers per rarity tier: e.g. Legendary = 40000 (4×), Epic = 30000 (3×), Rare = 20000 (2×), Common = 10000 (1×). Calls setRarityMultiplierBps() on-chain." },
      { step: 3, title: "Set Genesis Bonus", detail: "Configure the Genesis Bonus BPS (default 5000 = +50%). Wave 1 (Genesis) NFTs earn extra points on top of their rarity rate. Calls setGenesisBonusBps() on-chain." },
      { step: 4, title: "Holder Stakes NFT", detail: "Holder approves the BearthStaking contract and stakes via the Bearth platform. NFT transfers to the staking contract. Points accumulate automatically per day at their rarity-adjusted rate." },
      { step: 5, title: "Holder Claims Points", detail: "Holder calls claimPoints(tokenId) or claimAllPoints() to update their on-chain points balance without unstaking. Points balance visible in the holder's staking dashboard." },
      { step: 6, title: "Admin Redeems Points for BRT Tokens", detail: "Admin goes to NFT Staking → Redeem Points. Enter the holder wallet, points amount, and reason. Calls redeemPoints() on-chain which automatically mints BRT tokens to the holder." },
      { step: 7, title: "Unstake to Sell", detail: "To sell on OpenSea, holder calls unstake(). NFT returns to their wallet. All earned points are retained. BRT tokens already minted remain in their wallet." },
    ],
    ...
    tips: [
      ...
      "The Genesis bonus (+50% default) makes Wave 1 holders especially loyal — they earn significantly more than later waves.",
    ...
    <div className="flex h-full overflow-hidden" style={{ background: "#f0f2f7" }}>
      ...
      >
        ...
        <div className="flex-shrink-0 px-4 pt-4 pb-3 space-y-2" style={{ borderBottom: "1px solid #e5e7eb" }}>
          <div>
            ...
            <p className="text-xs" style={{ color: "#9bafc5" }}>
              {apiLoading ? "Loading…" : `${strategies.length} strategies ranked by industry adoption · ${strategies.filter(s => s.supported).length} live · Select a strategy, then click "Use This Strategy" to configure waves`}
            </p>
          </div>
          <input
            type="text"
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {apiError && (
            <div className="m-4 p-3 rounded-xl text-xs" style={{ background: "#fef2f2", border: "1px solid #fecaca", color: "#dc2626" }}>
              {apiError}
            </div>
          )}
          {!apiLoading && !apiError && (<>
